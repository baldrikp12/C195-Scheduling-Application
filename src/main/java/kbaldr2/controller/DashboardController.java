...
public class DashboardController implements Initializable {
    ...
    private void fifteenMinuteCheck() {
        ...
        for (DataCache item : DataCache.getAllAppointments()) {
            ...
            if (Math.abs(ChronoUnit.MINUTES.between(now, appointmentDateTime)) <= 15) {
                Alerts.showAlert("There is an appointment soon", "15 Minute Reminder");
                System.out.println("there is an aappoint");
    ...
    @FXML private void logout() throws IOException {
        //clear all the objects and lists in case we log back in...
